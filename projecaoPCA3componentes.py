...
def main():
    ...
    dataframe_principal = pd.DataFrame(data = componentes_principais[:,0:3], columns = ['Componente Principal 1', 'Componente Principal 2', 'Componente Principal 3'])
...
def VisualizarProjecaoPCA(dataframe_final, coluna_alvo):
    ...
    for alvo, cores in zip(alvos, cores):
        ...
        ax.scatter(dataframe_final.loc[indices_a_manter, 'Componente Principal 1'], dataframe_final.loc[indices_a_manter, 'Componente Principal 2'], dataframe_final.loc[indices_a_manter, 'Componente Principal 3'], c = cores, s = 50)
    ax.legend(alvos)
    ax.grid()
    plot.show()
